...
(function () {
  ...
  const translator = {
    ...
    async apply(root) {
      if (state.userLanguage === 'en') return;
      const attrs = ['title', 'placeholder', 'aria-label'];
      const SKIP = new Set(['SCRIPT', 'STYLE', 'TEXTAREA', 'CODE', 'PRE', 'SVG', 'PATH']);
      const walk = async (node) => {
        if (node.nodeType === Node.TEXT_NODE) {
          const orig = node.textContent;
          const trimmed = orig.trim();
          if (trimmed) {
            const trans = await translator.translate(trimmed);
            node.textContent = orig.replace(trimmed, trans);
          }
          return;
        }
        ...
        for (const a of attrs) {
          if (node.hasAttribute(a)) {
            ...
            if (v) node.setAttribute(a, await translator.translate(v));
          }
        }
  ...
  `;
    ...
    translator.apply(ui.mainContainer).catch(() => {});
